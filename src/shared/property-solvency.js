...
export function renderPropertyFilter(cashFlow, hidden = false) {
  ...
  return `<div class="property-solvency-control"><label><input type="checkbox" data-property-solvency ${cashFlow.includeRealEstateInSolvency !== false ? 'checked' : ''} /> Considerar imóveis na solvência patrimonial</label><p>Aplica-se ao patrimônio do gráfico e à visão geral. O caixa disponível continua igual.${excluded ? ` ${excluded} imóvel(is) excluído(s) individualmente permanecem fora.` : ''}</p>${!properties.length ? '<p>Nenhum bem está classificado como imóvel.</p>' : ''}${unclassified ? `<p>${unclassified} bem(ns) sem tipo informado. Classifique-os para que o filtro reconheça os imóveis.</p>` : ''}<a href="/patrimonio" data-route>Escolher imóveis e revisar bens</a></div>`
}
