...
import { cashFlowDetailIndex, renderCashFlowDetailNavigation } from './cash-flow-detail.js'
...
export function planningChart({ title, rows, series, bands = [], currency, hidden, markers = [], details = [], selectedYear, linkedSelection = false, interpolation = 'monotone', annualReadout = false, readoutCaption = '' }) {
  ...
  const hasDetails = details.length === rows.length && details.every(Boolean)
  const initialIndex = cashFlowDetailIndex(hasDetails ? details : rows.map(row => ({ label: period(row) })), selectedYear)
  const detailNavigation = hasDetails ? renderCashFlowDetailNavigation(details, initialIndex) : ''
  const fixedSelection = annualReadout || Boolean(detailNavigation) || linkedSelection && selectedYear != null
  ...
  const paths = key => {
    ...
    return segments.map(points => interpolation === 'linear' ? points.map(([px, py], index) => `${index ? 'L' : 'M'}${number(px)},${number(py)}`).join(' ') : planningChartLine(points)).join(' ')
  }
  ...
  const lines = series.map((item, seriesIndex) => item.type === 'bar' ? '' : `<g data-chart-series="${seriesIndex}"><path d="${paths(item.key)}" fill="none" stroke="${color(item.color)}" stroke-width="${Number.isFinite(item.width) ? Math.max(1, Math.min(6, item.width)) : 2.5}" stroke-dasharray="${dash(item.dash)}"/>${rows.map((row, index) => !finite(row[item.key]) ? '' : `<circle cx="${x(index)}" cy="${y(row[item.key])}" r="${rows.length === 1 ? 3 : 4}" fill="${rows.length === 1 || item.emphasize === true && index === rows.length - 1 ? color(item.color) : 'transparent'}"><title>${escapeHtml(period(row))} · ${escapeHtml(item.label)}: ${escapeHtml(money(row[item.key]))}</title></circle>`).join('')}</g>`).join('')
  ...
  const readouts = rows.map((row, index) => `<div data-chart-snapshot="${index}" ${annualReadout && index === initialIndex ? '' : 'hidden'}><h3>${row.year !== undefined ? 'Ano ' : ''}${escapeHtml(period(row))}</h3><dl>${[...series, ...bands].map((item, seriesIndex) => `<div data-chart-readout-series="${seriesIndex}"><dt><span class="planning-chart-readout-dot" style="background:${color(item.color)}" aria-hidden="true"></span>${escapeHtml(item.label)}</dt><dd>${item.key && item.monthly ? moneyWithMonthly(value => escapeHtml(money(value)), row[item.key], row.months) : escapeHtml(item.key ? money(row[item.key]) : `${money(row[item.low])} a ${money(row[item.high])}`)}</dd></div>`).join('')}</dl></div>`).join('')
  const legendHtml = `<ul class="planning-chart-legend" aria-label="Séries do gráfico">${legend}</ul>`
  const table = annualReadout ? `<details class="disclosure annual-chart-table"><summary>Consultar valores de todos os anos</summary><div class="table-scroll" tabindex="0" role="region" aria-label="Tabela: ${escapeHtml(title)}"><table><caption>${escapeHtml(readoutCaption)}</caption><thead><tr><th scope="col">Ano</th>${series.map(item => `<th scope="col">${escapeHtml(item.label)}</th>`).join('')}</tr></thead><tbody>${rows.map(row => `<tr><th scope="row">${escapeHtml(period(row))}</th>${series.map(item => `<td>${escapeHtml(item.monthly ? moneyWithMonthlyText(money, row[item.key], row.months) : money(row[item.key]))}</td>`).join('')}</tr>`).join('')}</tbody></table></div></details>` : ''
  const kind = bands.length ? 'Faixas de cenários e linhas' : barSeries.length ? barSeries.length === series.length ? 'Gráfico de barras' : 'Barras e linhas' : 'Gráfico de linhas'
  return `<figure class="planning-chart" data-planning-chart ${annualReadout ? 'data-chart-inline-readout' : ''} ${detailNavigation ? 'data-chart-drilldown' : ''} ${fixedSelection ? `data-chart-selected-index="${initialIndex}" data-chart-index="${initialIndex}"` : ''} data-chart-count="${rows.length}"><figcaption>${escapeHtml(title)}</figcaption><p class="planning-chart-help">${kind}. Passe o ponteiro, toque ou use as setas para comparar um período. Clique na legenda para filtrar séries, sem alterar a escala ou os cálculos.</p>${annualReadout ? legendHtml : ''}<div class="planning-chart-stage"><div class="planning-chart-scroll" tabindex="0" role="region" aria-label="${escapeHtml(title)}. Setas esquerda e direita escolhem o período. Home e End vão ao início e ao fim. ${annualReadout ? 'Escape volta ao ano selecionado. Enter ou Espaço fixa o ano.' : detailNavigation ? 'Escape fecha a prévia. Enter ou Espaço fixa o ano na composição.' : 'Escape fecha os valores.'}"><svg viewBox="0 0 ${plot.width} ${plot.height}" role="img" aria-label="${escapeHtml(title)}. Consulte os valores por período ou a tabela.">${min < 0 ? `<rect x="${plot.left}" y="${y(0)}" width="${plot.right - plot.left}" height="${plot.bottom - y(0)}" fill="#fff7f7"/>` : ''}${grid}${areas}<line class="planning-chart-zero" x1="${plot.left}" x2="${plot.right}" y1="${y(0)}" y2="${y(0)}" stroke="${min < 0 ? '#b45309' : '#94a3b8'}" stroke-width="${min < 0 ? 1.5 : 1}"/>${bars}${lines}${marks}${ticks}<line data-chart-cursor x1="${x(fixedSelection ? initialIndex : 0)}" x2="${x(fixedSelection ? initialIndex : 0)}" y1="${plot.top}" y2="${plot.bottom}" stroke="#334155" stroke-dasharray="4 3" visibility="${fixedSelection ? 'visible' : 'hidden'}" pointer-events="none"/></svg></div><div class="planning-chart-readout" data-chart-readout aria-live="polite" aria-atomic="true" ${annualReadout ? '' : 'hidden'}>${annualReadout ? `<p class="annual-readout-context"><strong data-chart-readout-state>Ano selecionado</strong> · ${escapeHtml(readoutCaption)}</p>` : ''}${readouts}</div></div>${annualReadout ? '' : legendHtml}${table}${detailNavigation}</figure>`
}
