...
export function planningChart({ title, rows, series, bands = [], currency, hidden, markers = [], details = [], selectedYear, linkedSelection = false, interpolation = 'monotone' }) {
  ...
  const initialIndex = cashFlowDetailIndex(details.length ? details : rows.map(row => ({ label: period(row) })), selectedYear)
  const detailNavigation = details.length === rows.length ? renderCashFlowDetailNavigation(details, initialIndex) : ''
  const fixedSelection = Boolean(detailNavigation) || linkedSelection && selectedYear != null
