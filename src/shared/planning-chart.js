...
export function planningChart({ title, rows, series, bands = [], currency, hidden, markers = [] }) {
  ...
  const pointHints = series.filter(item => item.type !== 'bar').map(item => rows.map((row, index) => `<circle cx="${x(index)}" cy="${y(row[item.key])}" r="5" fill="transparent"><title>${escapeHtml(row.year || row.month)} · ${escapeHtml(item.label)}: ${escapeHtml(privateCurrency(row[item.key], false, true, currency))}</title></circle>`).join('')).join('')
