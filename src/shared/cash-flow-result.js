...
export function renderCashFlowResult(row, currency, { final = false } = {}) {
  ...
  return `<section class="cash-flow-year-result ${result < -0.005 ? 'cash-flow-year-result--negative' : 'cash-flow-year-result--positive'}" aria-label="Resultado do período">
    ...
    <dl><div class="cash-flow-year-result-primary"><dt>${hasReturn ? 'Resultado final do ano' : 'Saldo do orçamento, antes dos rendimentos'}</dt><dd>${money(result)}</dd></div>${Number.isFinite(row.netWorth) ? `<div><dt>Patrimônio total líquido de dívidas ao fim do ano</dt><dd>${money(row.netWorth)}</dd></div>` : ''}${hasReturn ? `<div><dt>Patrimônio financeiro ao fim do ano</dt><dd>${money(row.financialAssets)}</dd></div><div><dt>Liquidez ao fim do ano</dt><dd>${money(row.liquidAssets)}</dd></div>` : ''}</dl>
    ${Number.isFinite(row.netWorth) ? `<p>${row.netWorth < -0.005 ? 'As dívidas superam o patrimônio total neste fechamento.' : row.liquidAssets < -0.005 ? 'Neste fechamento, falta liquidez para cobrir o orçamento. Bens ou saldos bloqueados precisam ficar disponíveis para pagar despesas.' : result < -0.005 ? 'O resultado anual negativo consome patrimônio. Isso, por si só, não significa insolvência.' : 'Patrimônio total inclui bens e saldos restritos. Esses valores dependem de disponibilidade para pagar despesas.'}</p>` : ''}
