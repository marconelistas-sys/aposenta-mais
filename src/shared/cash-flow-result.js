...
export function renderCashFlowResult(row, currency, { final = false } = {}) {
  ...
  return `<section class="cash-flow-year-result ${wealth < -0.005 || row.liquidAssets < -0.005 ? 'cash-flow-year-result--negative' : result < -0.005 ? 'cash-flow-year-result--consuming' : 'cash-flow-year-result--positive'}" aria-label="Resultado do período">
    ...
    <dl><div class="cash-flow-year-result-primary"><dt>${hasReturn ? 'Variação do patrimônio financeiro no ano' : 'Saldo do orçamento, antes dos rendimentos'}</dt><dd>${money(result)}</dd></div>${Number.isFinite(wealth) ? `<div><dt>${escapeHtml(solvencyWealthLabel(row))} ao fim do ano</dt><dd>${money(wealth)}</dd></div>` : ''}${hasReturn ? `<div><dt>Patrimônio financeiro ao fim do ano</dt><dd>${money(row.financialAssets)}</dd></div><div><dt>Liquidez ao fim do ano</dt><dd>${money(row.liquidAssets)}</dd></div>` : ''}</dl>
