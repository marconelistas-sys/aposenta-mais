...
export function renderProjectionChecks(result) {
  ...
  const list = (messages, details = []) => `<ul class="projection-review-list">${messages.map((message, index) => {
    const item = details[index]?.message === message ? details[index] : { message, href: '/viabilidade', action: 'Revisar premissas da avaliação' }
    const href = /^\/(?!\/)/.test(item.href || '') ? item.href : '/viabilidade'
    return `<li><p>${escapeHtml(message)}</p><a href="${escapeHtml(href)}" data-route>${escapeHtml(item.action || 'Revisar este dado')}</a></li>`
  }).join('')}</ul>`
  ...
  const required = pending.length || confirmations.length
    ? `<details class="disclosure projection-checks"><summary>Ver verificações da projeção</summary>${pending.length ? `<h3>Dados a revisar (${pending.length})</h3>${list(pending, result.issueDetails)}` : ''}${confirmations.length ? `<h3>Premissas a confirmar (${confirmations.length})</h3><p>São confirmações sobre como interpretar os dados cadastrados.</p>${list(confirmations, result.confirmationDetails)}${forms}` : ''}<a href="/viabilidade" data-route>Conferir dados e premissas</a></details>`
    : ''
  const information = notices.length ? `<details class="disclosure projection-notices"><summary>Como os saldos restritos entram no cálculo</summary>${list(notices, result.noticeDetails)}<p>Esses avisos explicam o tratamento adotado e não contam como pendências. Informe a liberação se pretende usar esses recursos.</p></details>` : ''
