...
function renderComposition(entries, money) {
  ...
  return `<dl class="wealth-callout-components">${entries.map(item => `<div><dt>${escapeHtml(item.name)}${item.category ? `<small>${escapeHtml(item.category)}</small>` : ''}</dt><dd class="money-value">${money(item.amount)}</dd></div>`).join('')}</dl>`
}

function bar(key, label, total, entries, maxValue, year, money) {
  ...
  return `<div class="remaining-wealth-row" data-wealth-row data-negative="${total < 0}">
    <button type="button" class="remaining-wealth-label" data-wealth-trigger aria-expanded="false" aria-label="Ver composição: ${label} em ${escapeHtml(year)}"><span>${label} <span class="wealth-info-icon" aria-hidden="true">ⓘ</span></span><strong class="money-value">${money(total)}</strong></button>
...
export function renderIncomeCostBars(row, currency) {
  ...
  return `<section class="remaining-wealth" aria-label="Receitas e despesas em ${escapeHtml(row.year)}">
    ...
    ${bar('income', 'Receitas', model.income, incomeItems, maxValue, row.year, money)}
    ${bar('costs', 'Despesas e metas', model.outflows, costItems, maxValue, row.year, money)}
  </section>`
}
