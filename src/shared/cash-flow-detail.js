...
export function cashFlowDetailPanels(rows, plan, currency, hidden = false) {
  if (hidden) return []
  ...
  return rows.map(row => {
    ...
    const group = (key, title, total, empty) => `<section class="cash-flow-detail-group"><h4>${title} <span>${money(total)}</span></h4>${row.breakdown[key].length ? `<div class="table-scroll" role="region" tabindex="0" aria-label="${title} de ${escapeHtml(row.year)}"><table><thead><tr><th scope="col">Lançamento e origem</th><th scope="col">Total no período</th></tr></thead><tbody>${row.breakdown[key].map(item => `<tr><th scope="row"><details><summary>${escapeHtml(item.name)}</summary><p>${escapeHtml(item.category)} · ${escapeHtml(item.source)}. ${escapeHtml(frequencies[item.frequency] || item.frequency)}. Incluído em ${item.months} mês(es).</p><p>Total na moeda original: ${privateCurrency(item.originalAmount, false, true, item.currency)}.</p>${item.startDate || item.endDate || item.retirement ? `<p>Início: ${escapeHtml(item.startDate || 'Sem data inicial')}. Fim: ${escapeHtml(item.retirement ? `Antes da aposentadoria em ${item.retirement}` : item.endDate || 'Sem data final')}.</p>` : ''}</details></th><td>${money(item.amount)}</td></tr>`).join('')}</tbody></table></div>` : `<p>${empty}</p>`}</section>`
    const income = row.income, costs = row.costs, goals = row.goals, pension = row.pensionCredits
    const html = `<h3>Composição de ${escapeHtml(label)}</h3>${renderCashFlowResult(row, currency)}<p>Receitas ${money(income)} menos custos ${money(costs)} e metas ${money(goals)} = saldo do orçamento ${money(row.freeCashFlow)}. Totais em ${escapeHtml(currency)}, pelos mesmos lançamentos do gráfico. ${row.priceBasis ? `Valores ${row.priceBasis === 'nominal' ? 'nominais de cada ano' : `reais de ${row.priceBaseYear}`}. Totais na moeda original abaixo permanecem na base real do cadastro.` : ''}</p>
      <div class="cash-flow-detail-grid">${group('income', 'Receitas', income, 'Nenhuma receita prevista neste período.')}${group('costs', 'Despesas, sem metas', costs, 'Nenhuma despesa prevista neste período. Confira se essa ausência é intencional.')}${group('goals', 'Metas', goals, 'Nenhuma meta prevista neste período.')}</div>
      ...
      <p>Valores anuais são provisionados nos meses ativos. Ampliar o horizonte não renova lançamentos encerrados. <a href="/fluxo-caixa" data-route>Revisar lançamentos no orçamento</a>.</p>`
    return { label, html }
  })
}
