...
import { renderIncomeCostBars } from './income-cost-bars.js'
...
export function cashFlowDetailPanels(rows, plan, currency, hidden = false, { wealthTargetAge = plan.targetAge } = {}) {
  ...
  return rows.map(row => {
    ...
    const group = (key, title, total, empty) => `<section class="cash-flow-detail-group"><h4>${title} <span class="money-value">${key === 'releases' ? money(total) : moneyWithMonthly(money, total, row.months)}</span></h4>${row.breakdown[key].length ? `<div class="table-scroll" role="region" tabindex="0" aria-label="${title} de ${escapeHtml(row.year)}"><table><thead><tr><th scope="col">Lançamento e origem</th><th scope="col">Total no período e valor mensal</th></tr></thead><tbody>${[...row.breakdown[key]].sort((a, b) => b.amount - a.amount || a.name.localeCompare(b.name, 'pt-BR')).map(item => `<tr><th scope="row"><details><summary>${escapeHtml(item.name)}</summary><p>${escapeHtml(item.category)} · ${escapeHtml(item.source)}. ${escapeHtml(frequencies[item.frequency] || item.frequency)}. Incluído em ${item.months} mês(es).</p><p>Total na moeda original: ${privateCurrency(item.originalAmount, false, true, item.currency)}.</p>${item.startDate || item.endDate || item.retirement ? `<p>Início: ${escapeHtml(item.startDate || 'Sem data inicial')}. Fim: ${escapeHtml(item.retirement ? `Antes da aposentadoria em ${item.retirement}` : item.endDate || 'Sem data final')}.</p>` : ''}${key === 'costs' ? expenseImpactAction(item, row, plan.targetAge) : ''}</details></th><td><span class="money-value">${item.frequency === 'occasional' || key === 'releases' ? money(item.amount) : moneyWithMonthly(money, item.amount, item.months)}</span></td></tr>`).join('')}</tbody></table></div>` : `<p>${empty}</p>`}</section>`
    const income = row.income, costs = row.costs, goals = row.goals, pension = row.pensionCredits
    const html = `<h3>Composição de ${escapeHtml(label)}</h3>${renderIncomeCostBars(row, currency)}${renderRemainingWealth(row, currency, wealthPeaks)}${renderBudgetComparison(row, currency)}${renderBudgetPressure(row, currency, { targetAge: plan.targetAge, showSummary: false })}<details class="disclosure"><summary>Como o saldo afeta o patrimônio neste período</summary>${renderCashFlowResult(row, currency)}<p>Receitas ${moneyWithMonthlyText(money, income, row.months)} menos custos ${moneyWithMonthlyText(money, costs, row.months)} e metas ${moneyWithMonthlyText(money, goals, row.months)} = saldo do orçamento ${moneyWithMonthlyText(money, row.freeCashFlow, row.months)}. Totais em ${escapeHtml(currency)}, pelos mesmos lançamentos do gráfico. ${row.priceBasis ? `Valores ${row.priceBasis === 'nominal' ? 'nominais de cada ano' : `reais de ${row.priceBaseYear}`}. Totais na moeda original abaixo permanecem na base real do cadastro.` : ''}</p></details>
      ...
      <p>Valor mensal dos totais = total dividido pelos meses incluídos no ano. Valor mensal de cada lançamento = total dividido pelos meses em que ele está ativo. Valores anuais são provisionados nos meses ativos. Ampliar o horizonte não renova lançamentos encerrados. <a href="/orcamento" data-route>Revisar lançamentos no orçamento</a>.</p>`
    return { label, html }
  })
}
