...
export function renderBudgetPressure(row, currency, { limit = 5, monthly = false, targetAge = null, showSummary = true } = {}) {
  ...
  return `<section class="budget-pressure" aria-label="Pressão dos gastos no orçamento">${showSummary ? `<div class="pressure-totals"><div><span>Receitas previstas</span><strong class="money-value">${monthly ? money(model.income) : moneyWithMonthly(money, model.income, model.months)}</strong></div><div><span>Saídas, despesas e metas</span><strong class="money-value">${monthly ? money(model.outflows) : moneyWithMonthly(money, model.outflows, model.months)}</strong></div><div data-tone="${model.balance < -0.005 ? 'negative' : 'neutral'}"><span>Saldo antes dos rendimentos</span><strong class="money-value">${monthly ? money(model.balance) : moneyWithMonthly(money, model.balance, model.months)}</strong></div></div>` : ''}
    ...
    ${rest > 0 ? `<p class="pressure-rest">Demais saídas: <span class="money-value">${monthly ? money(rest) : moneyWithMonthly(money, rest, model.months)}</span> · ${percent(rest / model.outflows)} das saídas.${monthly ? ' O cadastro abaixo pode estar filtrado e incluir realizados.' : ' Consulte a lista completa abaixo.'}</p>` : ''}
