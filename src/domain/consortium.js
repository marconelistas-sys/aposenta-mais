...
export function consortiumSchedule(item, horizon = 720, awardOverride = null) {
  ...
  for (let index = 0; index < horizon; index++) {
    ...
    const cashExpense = round(common + admin + reservePayment + insurancePayment + ownBid + topUp)
    // Where each unit of the installment goes. Savings become linked wealth
    // (restrictedEquity). Costs never come back. A service purchase is consumed.
    const savingsOutflow = round(common + ownBid + (item.useType === 'asset' ? topUp : 0))
    const consumptionOutflow = item.useType === 'service' ? round(topUp) : 0
    const costOutflow = round(cashExpense - savingsOutflow - consumptionOutflow)
    rows.push({ month, stage, common, administration: admin, reserve: reservePayment, insurance: insurancePayment, ownBid, embeddedBid, topUp,
      cashExpense, savingsOutflow, costOutflow, consumptionOutflow,
...
export const consortiumPhases = Object.freeze({
  pending: { title: 'Antes da contemplação', short: 'Pagando a cota', text: 'Você paga todo mês. A parte do fundo comum forma sua cota. O restante é custo do consórcio.' },
  credit: { title: 'Contemplado', short: 'Carta de crédito', text: 'Você tem uma carta de crédito. Ela só serve para comprar o bem ou serviço, não é dinheiro livre.' },
  asset: { title: 'Bem adquirido', short: 'Bem comprado', text: 'O crédito virou um bem. As parcelas continuam até o fim do prazo.' }
})

// Plain-language reading of one consortium at a month, from the same schedule used by the plan.
export function consortiumSummary(item, asOfMonth = item.referenceMonth) {
  const horizon = Math.max(item.months, item.useMonth ? monthOffset(item.referenceMonth, item.useMonth) + 1 : 1)
  const rows = consortiumSchedule(item, horizon)
  const index = Math.min(rows.length - 1, Math.max(0, monthOffset(item.referenceMonth, asOfMonth)))
  const row = rows[index]
  const future = rows.slice(index)
  const sum = key => round2(future.reduce((total, current) => total + current[key], 0))
  return {
    month: row.month,
    phase: row.stage,
    installment: row.cashExpense,
    savings: row.savingsOutflow,
    costs: round2(row.costOutflow + row.consumptionOutflow),
    savingsShare: row.cashExpense > 0 ? row.savingsOutflow / row.cashExpense : 0,
    linkedWealth: row.restrictedEquity,
    remainingToPay: sum('cashExpense'),
    remainingCosts: round2(sum('costOutflow') + sum('consumptionOutflow')),
    remainingSavings: sum('savingsOutflow'),
    endMonth: shiftMonth(item.referenceMonth, item.months - 1),
    awardMonth: item.stage === 'pending' ? item.awardMonth : null,
...
// Finds budget entries that probably repeat a consortium installment already
// generated by the Consórcios screen. Suspicion only, never an automatic change.
export function findConsortiumDuplicates(cashFlow, month) {
  const consortia = sanitizeConsortia(cashFlow?.consortia)
  if (!consortia.length) return []
  const matches = []
  for (const entry of cashFlow.items || []) {
    if (entry.type !== 'expense' || entry.recordKind === 'actual' || entry.source === 'txt' || entry.consortiumId) continue
    const text = normalizeText(`${entry.description || ''} ${entry.categoryId || ''}`)
    for (const item of consortia) {
      const index = monthOffset(item.referenceMonth, month)
      const installment = index >= 0 && index < item.months ? consortiumSchedule(item, index + 1)[index].cashExpense : 0
      const namedLike = text.includes('consorc') || (normalizeText(item.name).length >= 4 && text.includes(normalizeText(item.name)))
      const amountLike = installment > 0 && entry.currency === item.currency && entry.frequency === 'monthly' && Math.abs(entry.amount - installment) <= installment * 0.03
      if (namedLike || amountLike) {
        matches.push({ itemId: entry.id, itemName: entry.description || entry.categoryId, amount: entry.amount, currency: entry.currency, consortiumId: item.id, consortiumName: item.name, installment, reason: namedLike && amountLike ? 'name-and-amount' : namedLike ? 'name' : 'amount' })
        break
      }
    }
  }
...
function eventFromRow(item, row) {
  ...
  return { id: `consortium:${item.id}:${month}`, consortiumId: item.id, description: `${item.name}: parcela do consórcio`, categoryId: 'consortium', type: 'expense', amount: row.cashExpense, consortiumSavings: row.savingsOutflow, consortiumCosts: round2(row.costOutflow + row.consumptionOutflow), currency: item.currency, frequency: 'monthly', recordKind: 'planned', source: 'manual', startDate: `${month}-01`, endDate: `${month}-01`, date: `${month}-01` }
}
