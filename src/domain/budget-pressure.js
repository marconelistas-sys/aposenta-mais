...
export function budgetPressure(row) {
  ...
  const entries = ['costs', 'goals'].flatMap(group => (row.breakdown?.[group] || []).map(item => ({ ...item, group,
    kind: group === 'goals' ? 'Meta' : pensionIds.has(item.id) ? 'Previdência paga pelo orçamento' : item.source === 'Consórcio' ? 'Consórcio' : item.source === 'Compromisso' || item.budgetGroup === 'debt' ? 'Compromisso' : 'Despesa',
