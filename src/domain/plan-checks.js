...
export function planChecks(state, today = new Date()) {
  ...
  if (state.cashFlow.retirementMonth && state.cashFlow.retirementMonth !== retirementMonth(state.plan, today)) add('dates-differ', 'O mês confirmado do orçamento difere da estimativa pelas idades do plano. Revise os dois prazos.', '/construir/objetivo')
