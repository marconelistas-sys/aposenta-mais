...
export function saveAnnualPlanning(data) {
  ...
  const goalCategory = kind === 'annualGoals' ? categoryById(String(data.get('categoryId') || ''), state.customCategories) : null
  if (kind === 'annualGoals' && data.get('categoryId') && goalCategory?.type !== 'expense') throw new Error('Escolha uma categoria de despesa.')
  const saved = { ...row, ...(goalCategory ? { categoryId: goalCategory.id } : {}), ...(data.get('category') === 'unclassified' && kind === 'nonFinancialAssets' ? {} : { category }), ...(kind === 'nonFinancialAssets' && category === 'real-estate' ? { includeInSolvency: data.get('includeInSolvency') === 'on' } : {}) }
...
export function expenseCategoryOptions(selected = 'other-expense') {
  return categoriesForType('expense', state.customCategories).map(category => `<option value="${escapeHtml(category.id)}" ${category.id === selected ? 'selected' : ''}>${escapeHtml(category.name)}</option>`).join('')
}
...
export function renderAnnualPlanning(kind) {
  ...
  const goalCategoryField = assets ? '' : `<label class="form-field"><span>Categoria da despesa</span><select name="categoryId">${expenseCategoryOptions()}</select></label>`
