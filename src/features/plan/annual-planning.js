...
const categoryLabels = { 'real-estate': 'Imóvel', vehicle: 'Veículo', other: 'Outro bem' }
export function saveAnnualPlanning(data) {
  ...
  updateCashFlow({ [kind]: [...existing.filter(item => item.id !== id), { ...row, category }] })
}
export function renderAnnualPlanning(kind) {
  ...
  return `<section class="panel settings-card"><h2>${kinds[kind]}</h2><p>${assets ? 'Posições patrimoniais restritas, fora da Carteira. Entram no gráfico de risco sem gerar caixa e no Patrimônio consolidado. Início e fim do intervalo alteram o patrimônio, mas não representam compra ou venda financiada. Confira a origem do bem e evite cadastrá-lo também como investimento ou consórcio.' : 'Valor total do ano distribuído em 12 provisões mensais, sem presumir data de pagamento. Intervalo em anos e crescimento real contam desde o ano inicial. Não duplique estas provisões em despesas ou metas do calendário.'}</p><ul>${rows.map(row => `<li>${escapeHtml(row.name)}${assets ? ` <span class="badge badge--neutral">${categoryLabels[row.category] || categoryLabels.other}</span>` : ''} · ${privateCurrency(row.amount, state.valuesHidden, true, row.currency)} · ${row.startYear} a ${row.endYear}, a cada ${row.everyYears} ano(s)${state.valuesHidden ? '' : ` · ${(row.realGrowth * 100).toFixed(2)}% real ao ano <button class="button button--secondary" data-annual-edit="${row.id}" data-annual-kind="${kind}">Editar</button>`} <button class="button button--secondary" data-annual-remove="${row.id}" data-annual-kind="${kind}">Excluir</button></li>`).join('') || '<li>Nenhum registro.</li>'}</ul>${state.valuesHidden ? '' : `<details class="disclosure"><summary>Cadastrar ou editar</summary><form data-annual-planning="${kind}"><input type="hidden" name="kind" value="${kind}" /><input type="hidden" name="id" /><div class="form-grid form-grid--two"><label class="form-field"><span>Nome</span><input name="name" maxlength="60" required /></label><label class="form-field"><span>Moeda</span><select name="currency">${['BRL', 'CHF', 'EUR', 'USD'].map(code => `<option>${code}</option>`).join('')}</select></label>${categoryField}${field('amount', assets ? 'Valor no ano inicial' : 'Despesa total no ano inicial', 0.01, 1e9, 0.01)}${field('startYear', 'Ano inicial', 2000, 2199)}${field('endYear', 'Ano final, inclusive', 2000, 2199)}${assets ? '<input name="everyYears" type="hidden" value="1" />' : field('everyYears', 'Repetir a cada quantos anos', 1, 100, 1, 1)}${field('realGrowth', 'Crescimento real anual (%)', -99, 100, 0.01, 0)}</div><button class="button button--primary">Salvar</button><button class="button button--secondary" type="reset">Limpar</button></form></details>`}</section>`
}
