...
export function renderGuidedPlan(step = 'objetivo') {
  ...
  if (index === 0) content = `<form class="panel settings-card" data-guided-goal><h2>Quando e com qual renda?</h2><p>Valores em ${state.currency}, no poder de compra atual. As taxas são suas premissas, não garantias. Revise os valores sugeridos.</p><div class="form-grid form-grid--two">
    ...
    </div><p>O mês controla receitas vinculadas do orçamento. Revise-o ao mudar as idades, que controlam o prazo patrimonial. Benefício estimado não entra automaticamente nas receitas do orçamento.</p><div class="wizard-actions"><button class="button button--primary" type="submit">Salvar objetivo e continuar</button></div></form>`
